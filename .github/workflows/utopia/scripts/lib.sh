#!/usr/bin/env bash
# Utopia-cast-specific helpers, shared by the provision-* scripts in this
...
# Public hosts derive from the Utopia zone:
# <org>.utopia.playground.<network>.verana.network
utopia_zone() { echo "utopia.$(cast_zone)"; }

BUSINESS_REGISTRY_HOST="${R_BUSINESS_REGISTRY}.$(utopia_zone)"
CIVIL_REGISTRY_HOST="${R_CIVIL_REGISTRY}.$(utopia_zone)"
TAX_BURO_HOST="${R_TAX_BURO}.$(utopia_zone)"
MERIDIAN_BANK_HOST="${R_MERIDIAN_BANK}.$(utopia_zone)"
QUICKCASH_HOST="${R_QUICKCASH}.$(utopia_zone)"

# ---------------------------------------------------------------------------
# Governed-verification helpers (the Vesta cast never needed these: its
# schemas are all verifier-mode OPEN, while the Utopia Citizen ID is
...
# mode 3 (both ecosystem-governed): the Utopia Citizen ID relying-party rule.
