...
Deploys and provisions every verifiable service of the **Republica of Utopia
use case** (spec: `verana-spec/playground/utopia/spec.md`, §5) on the Verana
...
so the numbered `utopia-*.yml` entry points live there while everything else
...
— the demo-cast precedent; `scripts/lib.sh` adds the Utopia hosts and the
governed-verification helpers the other casts never needed.

## The cast and their domains

Orgs live at `<org>.utopia.playground.testnet.verana.network`.
...
| 03 | Civil Registry | identity authority (demo) | ECS-Org (Business Registry) + ECS-Service + Utopia Citizen ID registry (issuance AND verification ECOSYSTEM) + sole ISSUER on it |
...
`*.utopia.playground.testnet.verana.network` (a single wildcard only matches
...
ids (`utopia-citizen-id`, `utopia-legal-rep`) and the AnonCreds type names
(`UtopiaCitizenID`, `LegalRepresentative`) are the workflow contract of
`app/lib/utopia-cast.ts` and `/api/demo`.

## What CI/CD deliberately does not do

- **QuickCash never gets a VERIFIER permission on the Utopia Citizen ID.**
...
`app/lib/utopia-cast.ts` still carries the `QmUtopiaCastPending…`
