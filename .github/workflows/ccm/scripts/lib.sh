#!/usr/bin/env bash
# CCM-cast-specific helpers, shared by the provision-* scripts in this
# directory. Sourced AFTER vesta/common.sh (uses its helpers throughout —
# the demo/bolivia-cast lib.sh precedent).
...
R_CAMARA="camara"
R_BANCOLOMBIA="bancolombia"

# Public hosts derive from the CCM zone:
# <org>.ccm.playground.<network>.verana.network
ccm_zone() { echo "ccm.$(cast_zone)"; }

CAMARA_HOST="${R_CAMARA}.$(ccm_zone)"
BANCOLOMBIA_HOST="${R_BANCOLOMBIA}.$(ccm_zone)"

# ---------------------------------------------------------------------------
# Governed-verification helpers (copied from the Bolivia cast: the
# Representacion Legal schema is issuer-mode ECOSYSTEM AND verifier-mode
# ECOSYSTEM — the CCM's paid relying-party register: every verifier must
# register with the chamber, which is where the per-verification fee lives)
# ---------------------------------------------------------------------------
...
# mode 3 (both ecosystem-governed): the CCM relying-party rule.
