...
# self-issued ECS-Service, and the Utopia Citizen ID trust registry — both
...
# Utopia Citizen ID trust registry + governed schema + root permission + VTJSC
...
# app/lib/utopia-cast.ts looks this type up by name)
ensure_anoncreds_credential_type "$API" "UtopiaCitizenID" "1.0" "$CITIZEN_JSC_URL"
