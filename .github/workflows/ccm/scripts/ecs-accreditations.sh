#!/usr/bin/env bash
# ECS-side accreditation for the CCM cast. Runs with the ECS ecosystem
# controller account (ECS_ECOSYSTEM_MNEMONIC): the ECS-Organization schema is
# issuer-mode ECOSYSTEM, so only that account can accredit the Camara de
# Comercio de Medellin (demo) as an ECS-Organization issuer — the demo
# simplification of the target deployment, where the Verana Council would
# onboard Confecamaras as ISSUER_GRANTOR and Confecamaras would accredit the
# 57 chambers. Business IDs are plain ECS-Organization credentials issued by
# the register that IS the source of truth about Colombian companies.
...
CAMARA_DID=$(get_public_did_from_host "$CAMARA_HOST")
[ -n "$CAMARA_DID" ] || { err "Could not resolve the CCM DID from https://${CAMARA_HOST} — deploy ccm-01 first"; exit 1; }
ok "Camara de Comercio de Medellin DID: $CAMARA_DID"
...
if EXISTING=$(find_active_issuer_perm "$ORG_SCHEMA_ID" "$CAMARA_DID"); then
  ok "The CCM already holds an active ISSUER permission on ECS-Organization: $EXISTING"
else
  ...
  log "Granting the CCM an ISSUER permission on ECS-Organization (schema $ORG_SCHEMA_ID)..."
  ensure_validated_issuer_perm "$ORG_SCHEMA_ID" "$CAMARA_DID"
fi
